...
- **Without `?date=`**: compares **yesterday, today, and tomorrow** in `Europe/Amsterdam` for each configured segment, but **drops** calendar days before the earliest `departure_local` date present in **PlannedSegment** for those legs (so you do not write empty rows before the export exists). **FR24** tries **direct** GET → **Serper** scrape (`SERPER_API_KEY`) → **headless Chrome** when parses are still empty. The cron **closes the FR24 browser before Airfleets** starts its Chromium. **`FR24_PLAYWRIGHT=0`** disables FR24 Chromium only (no Airfleets change). Fetched **once per flight** per run. Override with `?date=YYYY-MM-DD` for a single-day backfill.
