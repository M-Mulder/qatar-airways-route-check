...
- **Without `?date=`**: compares **yesterday, today, and tomorrow** in `Europe/Amsterdam` for each configured segment, but **drops** calendar days before the earliest `departure_local` date present in **PlannedSegment** for those legs (so you do not write empty rows before the export exists). **FR24** uses **Serper scrape** when **`SERPER_API_KEY`** is set (recommended on Vercel). **Headless Chrome for FR24 is disabled on Vercel by default** — set **`FR24_PLAYWRIGHT=1`** only if you explicitly want it (often breaks in serverless). Locally, Playwright can still backfill FR24 when Serper returns no rows. Fetched **once per flight** per run. Override with `?date=YYYY-MM-DD` for a single-day backfill.
