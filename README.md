...
- **Vercel Cron** (once per day on Hobby) → `GET /api/cron/compare` (secured with `CRON_SECRET`). Each run may fetch **Airfleets.net** once per distinct tail for the registration hover on `/compare`. **Airfleets uses captcha / Cloudflare** — production cron on **Vercel** uses **HTTP-only** for Airfleets (often **403**; Playwright’s Chromium bundle exceeds Vercel’s **~250 MB** serverless size limit). **`npm run cron:local`** uses **Playwright Chromium** on your machine (`npx playwright install chromium`). Set `AIRFLEETS_BROWSER=0` to force HTTP-only locally. Cron **`maxDuration`** is **300s** in [`vercel.json`](vercel.json).
