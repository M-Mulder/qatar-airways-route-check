...
- **Vercel Cron** (once per day on Hobby) → `GET /api/cron/compare` (secured with `CRON_SECRET`). Each run may fetch **Airfleets.net** once per distinct tail for the registration hover on `/compare`. **Airfleets uses captcha / Cloudflare** — the cron uses **`playwright-core`** plus a small Linux Chromium from [**`@sparticuz/chromium`**](https://www.npmjs.com/package/@sparticuz/chromium) on **Vercel** (fits typical serverless size limits better than Playwright’s full browser download). Locally, the same code path uses **Google Chrome** via `channel: "chrome"` (install Chrome) or **`PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`**. Set **`AIRFLEETS_BROWSER=0`** to force fragile HTTP-only fetches. Cron **`maxDuration`** is **300s** in [`vercel.json`](vercel.json). In **Vercel function logs**, lines like **`[compare] Airfleets fetch start`** appear only when a row is about to be saved (Qsuite and equipment must both be decisive); short runs with only FR24 URLs mean every segment was skipped before Airfleets.
