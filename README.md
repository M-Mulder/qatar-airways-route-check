...
- **Without `?date=`**: compares **yesterday, today, and tomorrow** in `Europe/Amsterdam` for each configured segment, but **drops** calendar days before the earliest `departure_local` date present in **PlannedSegment** for those legs (so you do not write empty rows before the export exists). **FR24** tries a **direct** GET first (history is often SSR in `#tbl-datatable`); **`SERPER_API_KEY`** is used only when that response has no usable table (e.g. HTTP 403), because Serper’s scrape commonly returns marketing text instead of rows. **Headless Chrome for FR24 is disabled on Vercel by default** (`FR24_PLAYWRIGHT`). Fetched **once per flight** per run. Override with `?date=YYYY-MM-DD` for a single-day backfill.
